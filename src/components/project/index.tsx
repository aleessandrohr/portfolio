...
export const Project = ({
	...
	useEffect(() => {
		const syncDemoWithHash = () => {
			setIsDemoOpen(
				Boolean(iframeUrl) && window.location.hash === `#${id}-demo`
			)
		}

		syncDemoWithHash()

		window.addEventListener('hashchange', syncDemoWithHash)

		return () => window.removeEventListener('hashchange', syncDemoWithHash)
	}, [id, iframeUrl])

	return (
		<Card
			id={id}
			className="group relative flex w-full min-w-0 scroll-mt-24 flex-col overflow-hidden border-border/80 transition-all duration-300 hover:-translate-y-1 hover:border-brand/50 hover:shadow-brand/5 hover:shadow-xl lg:flex-row"
		>
			{iframeUrl && (
				<span
					id={`${id}-demo`}
					className="pointer-events-none absolute inset-x-0 top-0 h-px scroll-mt-24"
					aria-hidden="true"
				/>
			)}
